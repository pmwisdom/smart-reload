...
SmartReload = {
    _options: {
        ...
        splashScreenTimeout: 2000,
        splashWaitOnLayout: 'layout',
        debug: false
    },
    routes: {},
    selectors: {
...
function hideSplashScreen() {
    Meteor.isCordova && navigator && navigator.splashscreen && navigator.splashscreen.hide();
}
function showSplashScreen() {
    Meteor.isCordova && navigator && navigator.splashscreen && navigator.splashscreen.show();
}
...
function checkForPendingReload() {
    SmartReload.debug("Pending Reload? " + myRetry);
...
function onMigrate(retry) {
    ...
    if(SmartReload._options.reload.firstStart && !firstStart) {
        ...
        SmartReload.debug("Not First Start, lets start the idle tracker");
...
function startIdleTracker() {
    ...
    SmartReload.idleWatchTracker = Tracker.autorun(function () {
        ...
        SmartReload.debug("Idle Status Changed " + status);
